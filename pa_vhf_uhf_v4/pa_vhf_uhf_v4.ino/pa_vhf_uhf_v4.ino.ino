...
float vswr_thresh = 5.0; //Overcurrent Threshold, above = PA Overcurrent fault state

//PTT delay time, amount of time between each event
int16_t ptt_delay = 10;
...
void loop() {
  ...
  if (!cal_mode){
    if ((pa_fwd_pwr > 1.0) & (vswr > vswr_thresh) & (pa_state != 5)){ //not already in VSWR Fault State
      fault_vswr = vswr;
